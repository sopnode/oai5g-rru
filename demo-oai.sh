...
RFSIM_IMSI=@RFSIM_IMSI@ #value defined by configure-demo-oai.sh / mysql database issue
...
if [[ "$rru" == "jaguar" || "$rru" == "panther" ]]; then
    ...
    kubectl -c tcpdump cp $ns/$GNB_POD_NAME:nrL1_stats.log $prefix/gnb-L1_stats.log"$DATE" || true
    kubectl -c tcpdump cp $ns/$GNB_POD_NAME:nrMAC_stats.log $prefix/nrMAC_stats.log"$DATE" || true
    kubectl -c tcpdump cp $ns/$GNB_POD_NAME:nrRRC_stats.log $prefix/nrRRC_stats.log"$DATE" || true
fi
