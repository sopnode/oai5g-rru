...
# oai-nr-ue IMSI are specific cases, define them separatly
RFSIM_IMSI_UE="001010000001121"
...
if [ -f /etc/oai/5g-env.sh ]; then
    ...
    SLICE2_UPLINK="40Mbps"
    SLICE2_DOWNLINK="80Mbps"
...
function update() {
    ...
    CSI_FLUSH_CORE=${CSI_FLUSH_CORE:="32"}
...
s|@DEF_CSI_ANTENNA_SELECTION@|$CSI_ANTENNA_SELECTION|
s|@DEF_CSI_PORT_SELECTION@|$CSI_PORT_SELECTION|
s|@DEF_CSI_SUBCARRIER_SAMPLING@|$CSI_SUBCARRIER_SAMPLING|
s|@DEF_CSI_FLUSH_CORE@|$CSI_FLUSH_CORE|
...
s|@DEF_OPC@|${OPC}|g
s|@DEF_RFSIM_IMSI_UE@|${RFSIM_IMSI_UE}|g
